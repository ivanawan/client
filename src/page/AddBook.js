...
const AddBook = () => {
    ...
    function submit(){
      mutlipart.post('/book',form).then(res=>{
       check(res);
...
<input type="file" onChange={(e)=>{ setForm({...form , thumbnail:e.target.files[0]})}} className=' mt-3'/>
<label>ebook</label>
<input type="file" onChange={(e)=>{ setForm({...form , bookAttachment:e.target.files[0]})}} className=' mt-3'/>
