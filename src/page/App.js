            ...
            <a href={`/detail/${item.id}`} key={item.id}>
              <div className="w-40 mt-7">
                <img
                  ...
                  alt="sampul"
                />
                <p className=" text-base w-full font-semibold">
                  {item.title.substring(0, 20)}
                </p>
                <p className=" text-sm text-slate-400">by {item.author}</p>
                <p className="  text-lime-500 mt-3">
                  Rp. {formatUang(item.price)}
                </p>
              </div>
            </a>
          ))}
...
function PromoBooklogin() {
  ...
  let { data: myBook } = useQuery("profilChache", async () => {
          ...
          <div className=" w-3/5 px-2 my-5 bg-white relative">
            ...
            <div className=" absolute bottom-0 w-full">
              ...
              {myBook?.includes(item.id) ? (
                <a
                  href={`${process.env.REACT_APP_URL}/book/${user.id}/${item.id}`}
                  className="bg-slate-800  text-center inline-block text-sm  text-white w-full p-1"
                >
                  Download
                </a>
              ) : (
                <button
                  ...
                  className=" bg-slate-800  text-sm  text-white w-full p-1"
                >
                  Add to Cart
                </button>
              )}
